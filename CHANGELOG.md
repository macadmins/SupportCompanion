...
- **The helper only serves 3.0.0 and later clients.** A valid signature proves a connecting app is a Support Companion build signed by us, but every 2.x release satisfies that too — and in 2.x, `IsPrivileged` was honored from preference domains a standard user can write. Keeping a 2.x copy on disk and running it was therefore enough to reach the helper and get root, no matter what this version does. The helper now checks the connecting app's version, requires it to be signed with the hardened runtime, and requires it to run from `/Applications/SupportCompanion.app`, which a standard user cannot write to.
...
- The package's preinstall script ran `Uninstall.sh` from a fixed path inside a 1.x install as root. It now runs it only when that file is owned by root and not writable by group or others.
