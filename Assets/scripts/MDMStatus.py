#!/usr/local/supportcompanion/supportcompanion-python
...
def get_mdm_enrollment_time():
    ...
    try:
        ...
        profile_output = profile_output.split("\n")
        ...
        for val in profile_output:
            if "installationDate" in val:
                ...
                date = re.search(date_pattern, val)
...
def get_current_user():
    try:
        command = "echo 'show State:/Users/ConsoleUser' | scutil | awk '/Name :/ { print $3 }'"
        current_user = subprocess.Popen(
            command,
            shell=True,
            stdout=subprocess.PIPE,
        )
        current_user = current_user.stdout.read().strip()
    except Exception as e:
        print("Error: %s" % e)
        exit(1)
...
def write_status(status):
    ...
    with open(
        "/usr/local/supportcompanion/mdm_status.json",
        "w",
    ) as f:
